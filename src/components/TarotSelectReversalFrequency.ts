...
export default class TarotSelectBaseDeck extends Component {
    ...
    async execute(interaction: StringSelectMenuInteraction): Promise<void> {
        ...
        const selectedFrequency = parseInt(interaction.values[0], 10);
        userSettings.reversalFrequency = selectedFrequency;
        
        await updateTarotSettings(userId, userSettings);
        await interaction.reply({
            content: `Reversal frequency set to ${selectedFrequency}.`,
            ephemeral: true,
        });
    }
}